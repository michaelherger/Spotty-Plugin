...
sub _getNextTrack {
	...
	$spotty->playerNext(sub {
		# Delay querying Spotify's REST API state - it often lags behind the skip
		# command (which is processed much faster via Spirc/WebSocket) and still
		# reports the old track as "current". Querying immediately causes a false
		# positive on the "already played" check, triggering a premature stop.
...
sub _connectEvent {
	...
	# Handle seek events immediately using the position already known from the
	# Spirc event - avoids a REST API roundtrip that frequently returns stale
	# state (the API lags 500ms+ behind the WebSocket path Spirc uses).
	# This must be checked before the newTrack guard to avoid accidentally
	# clearing that flag during a seek.
	if ( $cmd eq 'seek' ) {
		my $position = $request->getParam('_p2');
		if ( defined $position && $client->isPlaying && __PACKAGE__->isSpotifyConnect($client) ) {
			main::INFOLOG && $log->is_info && $log->info("Got a seek event from Spotify - seeking LMS to ${position}s");
